...
export default function Chats() {
  ...
  const handleSendMessage = (e) => {
    ...
    dispatch(setRecentMessage({chatId:currentChat._id,message:messgaeObj}))

    socket.emit("send:message", {message:messgaeObj,chat:currentChat, sender: {
      _id: user._id,
      name: user.name
    }})
    sendMessage(data)
  ...
  const handleGroupModalSubmit = (e) => {
    ...
    setLoader(true);
    dispatch(createCommunityChat({name: data.get("text"), members: selectedMembers.map((member) => member._id)}));
    if(!error){
      setSearch("");
      setSearch2("")
      setGroupModalVisible(false);
  ...
  return (
    ...
    <section className="flex h-screen antialiased text-gray-800">
      <div className="flex flex-row h-full w-full overflow-x-hidden">
        <aside className="flex flex-col py-8 pl-6 pr-2 w-72 bg-white flex-shrink-0">
          ...
          <div className="flex flex-col mt-8 overflow-y-scroll overflow-x-hidden">
            ...
            <div className="flex flex-col" id="dms">
              {chats?.map((chat) => (
                <Box key={chat._id} letter={
                  chat.members.find((user) => user._id !== currentUserId)?.name[0]
                } name={
                  chat.members.find((user) => user._id !== currentUserId)?.name
                } 
                avatar={chat.members.find((user) => user._id !== currentUserId)?.avatar}
                onClick={()=>handleOpenChat(chat.members.filter((user) => user._id !== currentUserId).map(u=>u._id),false,null)}
                highlight={chat._id === currentChat?._id}
                chatBox={true}
                recentMessage={chat.recentMessage?chat.recentMessage.content:""}
            ...
            <div className="flex flex-col" id="communities">
            {
              communityChats?.map((chat) => (
                <Box key={chat._id} letter={chat.name.slice(0,1).toUpperCase()} name={chat.name} onClick={()=>handleOpenChat(chat.members.filter((user) => user._id !== currentUserId).map(u=>u._id),true,chat._id)
                } highlight={chat._id === currentChat?._id
                }
                chatBox={true}
                recentMessage={chat.recentMessage?chat.recentMessage.content:""}
        ...
        {loader ?
          ...
          <div className="chat-box flex flex-col flex-auto flex-shrink-0 rounded-2xl bg-gray-100 h-full p-4">
            <div className="flex flex-col h-full overflow-x-auto mb-4">
                {/* create chat header */}
                <div className="flex flex-row items-center justify-between p-2  rounded-xl bg-gray-100 shadow-sm sticky">
                  <div className="flex flex-row items-center gap-2">
                   
                    <div className="flex flex-col items-center gap-2">
                      <span className="text-xl font-bold">
                        {currentChat?.isGroup?
                          <Box letter={currentChat.name[0].toUpperCase()} name={currentChat.name} />
                        :
                          <Box letter={currentChat?.members?.find((user) => user._id !== currentUserId)?.name[0]} name={currentChat?.members?.find((user) => user._id !== currentUserId)?.name} 
                          avatar={currentChat?.members?.find((user) => user._id !== currentUserId)?.avatar}
                          />
                        }
                      </span>
                      {currentChat?.isGroup &&
                ...
                <div className="flex flex-row items-center gap-2"
                  style={{
                    display:currentChat?.isGroup && currentChat?.admin === user._id ? "flex":"none"
                  }}
                >  
                <button
              ...
              <div className="flex flex-col h-full  overflow-y-scroll overflow-x-hidden" ref={msgRef}>
                <div className="grid grid-cols-12 gap-y-2 " >
                  {currentChatMessages.map((message) => (
                    (message.sender === user._id || message.sender?._id===user._id) ? 
                    <MessageMine key={message._id} letter={message.sender.name?message.sender.name[0]?.toUpperCase():user.name[0].toUpperCase()} message={message.content} type={message.type} /> :
                    <MessageOther key={message._id} letter={message.sender?.name[0]?.toUpperCase()} message={message.content} type={message.type}
            ...
            <div className="flex flex-row items-center gap-2 h-16 rounded-xl bg-white w-full px-2">
              <form className="flex flex-row items-center w-full" onSubmit={handleSendMessage}>

              <div className="message-box flex-grow">

                  <MessageInput value={message} onChange={(e) => {
                    setMessageType("text")
                    setMessage(e.target.value)}} placeholder="Type your message..." fileAllowed={true} onFileChange={(e) => {
                    setMessageType("file")
                    setFile(e.target.files[0])}} disabled={file!==null}/>
              </div>
              <button type="submit" className="flex items-center justify-center bg-primary-500 hover:bg-primary-600 rounded-xl text-white px-4 py-2 flex-shrink-0"
                >
                <span>Send</span>
                <span className="ml-2">
                  <svg
        ...
        {!loader && !currentChat &&
          <div className="w-full h-full flex items-center justify-center flex-col gap-4">
            {/* crop svg */}
            <svg fill="rgb(46 204 113)" height="100px" width="100px" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"  
              viewBox="0 0 60 60">
            ...
            <span className="text-2xl font-bold text-gray-400">Select a chat to start messaging</span>
          </div>
        }
          ...
          <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div className="p-4 md:p-5">
              <form className="space-y-4" action="#">
                ...
                <div>
                <div
                  className="flex flex-col gap-2 overflow-y-auto overflow-x-hidden max-h-72  "
                >
                  {
                    filteredUsers.filter(user=>{
                      return user._id !== currentUserId && !selectedMembers.includes(user)
                    }).map((user) => (
                      <Box key={user._id} letter={user.name.slice(0,1)} name={user.name} onClick={()=>handleOpenChat(user._id,false)}/>
                    ))
                  }
                </div>
                </div>
      ...
      >
        <div className="absolute top-8 left-1/2 -translate-x-1/2 p-4 w-full max-w-md max-h-full">
          ...
          <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div className="p-4 md:p-5">
              <form className="space-y-4" action="#" onSubmit={handleGroupModalSubmit}>
                <div>
                  <label
                    htmlFor="text"
                    className="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                ...
                <div>
                  ...
                  >
                    ...
                    {selectedMembers.map((member) => (
                      <button className={`flex items-center cursor-pointer justify-center p-2 rounded-sm  w-fit text-xs ${member._id !== user._id ? "bg-primary-500 text-white" : "bg-gray-300 text-gray-900"}`}
                      onClick={()=>setSelectedMembers(selectedMembers.filter((user) => user !== member))}
                ...
                >
                  {
                    filteredUsers.filter(user=>{
                      ...
                      <Box key={user._id} letter={user.name.slice(0,1)} name={user.name} onClick={()=>handleAddUsers(user)}/>
                    ))
                  }
                </div>
            ...
            <div className="p-4 md:p-5">
              <form className="space-y-4" action="#">
                ...
                <div className="flex flex-row gap-2 items-center flex-wrap">
                  {currentChat?.members.map((member) =>{
                    if(member._id !== user._id){
                      ...
                      return <div className="flex flex-row items-center gap-2 bg-primary-500 rounded-md px-2 py-1 text-white cursor-pointer"
                        onClick={()=>{
                      ...
                      >
                        ...
                        <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"  
	                          width="20" height="20" viewBox="0 0 64 64" enable-background="new 0 0 64 64" >
                            <g>
                ...
                <div className="flex flex-col gap-2 overflow-y-auto overflow-x-hidden max-h-72">
                  {filteredUsers.filter(user=>{
                      return user._id !== currentUserId && !currentChat?.members.includes(user)
                    }).map((user) => (
                    <Box key={user._id} letter={user.name.slice(0,1)} name={user.name} 
                      onClick={()=>{
